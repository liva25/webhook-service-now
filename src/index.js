...
const password =
  "S#)z&[B;3*g)*pe1D(ZRj!5[1Dj<fyXs3>bC}#4%q;-)oZ{j<r2tmolp+5n@*3:PYQ8s?},.DRRELT!Mv<OLS?-8ec{F}=A{Te:]";
...
app.post("/webhook", async (req, res) => {
  // webhook is a API, it is triggered by servicenow when a customer is trying to create a ticket
  console.log("Webhook received:", req.body);
  try {
    //to handle error, null check error, request undefined
    const response = await axios.post(
      `https://dev221314.service-now.com/api/now/table/incident?sysparm_limit=1`,
      req.body,
      {
        headers: {
...
app.listen(PORT, () => {
  //allow our application to run on a specific port
